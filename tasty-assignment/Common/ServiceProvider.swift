...
/// throughout the application. It acts as a centralized point for managing dependencies
/// and makes it easier to replace or mock these services during testing.
class ServiceProvider {
    ...
    init(watchlistService: WatchlistServiceProtocol, wastchlistStorage: WatchlistStorageProtocol, apiClient: APIClientProtocol) {
...
extension ServiceProvider {
    /// Creates a default instance of `ServiceProvider` with default implementations
    /// for each service and storage. Useful for standard application setup.
    static func defaultProvider() -> ServiceProvider {
        let apiClient = DefaultAPIClient()
        return ServiceProvider(
            watchlistService: WatchlistService(apiClient: apiClient),
            wastchlistStorage: WatchlistStorage(),
            apiClient: apiClient
        )
    }
}
...
/// to function within the application, specifically the services it needs to perform
/// its tasks.
...
/// Extending `ServiceProvider` to conform to `AppCoordinatorProvider` ensures that
/// it can be used by coordinators within the application.
